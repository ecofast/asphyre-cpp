...
void CASCEventProvider::UnSubscribeClass(const string& sClassName)
{
...
ASCBoolean CASCEventProvider::SetClassPriority(const string& sClassName, const ASCInt nPriority)
{
	for (ASCInt i = 0; i < m_Datas.size(); i++)
	{
		if (m_Datas[i].m_sClassName == sClassName)
...
void CASCEventProviders::UnSubscribe(const string& sClassName)
{
	for (ASCInt i = m_Datas.size() - 1; i >= 0; i--)
	{
